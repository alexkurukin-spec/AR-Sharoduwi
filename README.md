...
- **PBR-материалы** — clearcoat / metalness / roughness / emissive; отдельно
  «фольга» и «латекс».
- **Офлайн-окружение** — отражения на фольге собраны из `Lightformer`'ов, без
  загрузки HDRI из сети (drei `Environment preset=…` тянет `.hdr` с внешнего CDN
  и падает при офлайне). Плюс `ErrorBoundary`: сбой загрузки ассета не роняет
  overlay, а показывает аккуратный fallback.
